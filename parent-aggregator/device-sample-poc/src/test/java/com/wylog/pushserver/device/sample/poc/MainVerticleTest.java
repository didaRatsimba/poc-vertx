...
public class MainVerticleTest {
	...
	public void setUp(TestContext context) {
		...
		vertx = Vertx.vertx();
		vertx.deployVerticle(DevicePOCVerticle.class.getName(), ar -> {
			if (ar.succeeded()) {
				async.complete();
			} else {
	...
	public void testHello(TestContext context) {
		...
		//        });
		//        req.end();
		JsonObject object = new JsonObject().put("device_id", "ADFTGHJHJJY");
		vertx.eventBus().publish("device-poc", object);
	}
